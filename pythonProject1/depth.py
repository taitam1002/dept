...
def capture_image(cap):
    ...
    if not ret:
        ...
        return None
    return frame  # Sửa lỗi thiếu từ 'return'
...
search_params = dict(checks=100)  # Kiểm tra tối đa 100 lần
...
if len(good_matches) > 10:
    ...
    ransac_good_matches = sorted(ransac_good_matches, key=lambda x: x.distance)[:10]

    # Chọn điểm có độ khớp cao nhất (tức là điểm có distance nhỏ nhất)
    best_match = min(ransac_good_matches, key=lambda x: x.distance)

    # Vẽ chỉ điểm khớp tốt nhất trên ảnh màu với đường dày hơn và màu sắc rõ ràng
    img_matches = cv2.drawMatches(img1_color, kp1, img2_color, kp2, [best_match], None,
                                  matchColor=(0, 255, 0),  # Màu xanh lá cho các đường khớp
                                  singlePointColor=None,
                                  flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)

    # Vẽ đường kết nối giữa điểm tốt nhất
    pt1 = np.int32(kp1[best_match.queryIdx].pt)
    pt2 = np.int32(kp2[best_match.trainIdx].pt) + np.array([img1_color.shape[1], 0])
    ...
    plt.title("Điểm khớp tốt nhất với chấm tròn")
    plt.show()
else:
